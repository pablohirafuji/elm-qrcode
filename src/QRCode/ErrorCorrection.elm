...
get__ rsPoly dataCodewords =
    Result.map (get___ (Array.length rsPoly - 1))
        (mod (newPolynomial dataCodewords (Array.length rsPoly - 1)) rsPoly)


get___ : Int -> Polynomial -> List Int
get___ ecLength modPoly =
    Array.toList
        (Array.initialize ecLength
            (\index ->
                let
                    modIndex =
    ...
    let
        generate count polyResult =
            if count < ecLength then
                ...
                polyResult
    in
    generate 0 (Result.Ok (newPolynomial [ 1 ] 0))
...
newPolynomial num shift =
    ...
    Array.initialize
        ...
        (\index ->
            Maybe.withDefault 0 (Array.get (index + offset) numArray)
        )
    ...
    let
        ...
        process__ indexSum num_ exp =
            Result.fromMaybe PolynomialMultiplyException
                (Maybe.map (Bitwise.xor exp) (Array.get indexSum num_))
    in
    Result.map (\a -> newPolynomial a 0)
        (Result.map Array.toList
            (List.foldl process
                (Result.Ok num)
                (List.concat valuesArray)
            )
        )
    ...
    if Array.length poly1 - Array.length poly2 < 0 then
        ...
        let
            ...
            numResult =
                Array.foldl numFold
                    (Result.Ok poly1)
                    (Array.indexedMap (\a b -> ( a, b )) poly2)
            ...
            helper ( index2, value2 ) poly1_ =
                Result.map (\r -> Array.set index2 r poly1_)
                    (Result.andThen (helper_ index2 poly1_)
                        (Result.map getExp
                            (Result.map2 (+) ratio (getLog value2))
                        )
                    )
        ...
        Result.andThen (\a -> mod a poly2)
            (Result.map (\a -> newPolynomial a 0)
                (Result.map Array.toList numResult)
            )
...
getExp index =
    Maybe.withDefault 0
        (Array.get (modBy 255 index) expTable)
